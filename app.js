...
document.getElementById('form-registro').addEventListener('submit', e => {
  e.preventDefault();
  const mes = document.getElementById('reg-mes').value;         // YYYY-MM
  ...
  render();
  e.target.reset();
  // depois de criar, seleciona o mês recém-criado no simulador
...
document.getElementById('form-sim').addEventListener('submit', e => {
  ...
  // se marcada como executada, desmarca as outras do mesmo mês
  if (executada) {
...
// Delegação: toggle executada, delete sim, delete registro
...
document.getElementById('select-mes-sim').addEventListener('change', renderSimList);
...
document.getElementById('btn-cfg-cancel').addEventListener('click', () => modal.hidden = true);
document.getElementById('btn-cfg-save').addEventListener('click', () => {
  const parcela = parseFloat(document.getElementById('cfg-parcela').value);
  const saldoOrig = parseFloat(document.getElementById('cfg-saldo-original').value);
  const prazoOrig = parseInt(document.getElementById('cfg-prazo-original').value, 10);
  if (isNaN(parcela) || parcela <= 0) { alert('Informe uma parcela válida.'); return; }
  state.settings.parcela = parcela;
  state.settings.saldoOriginal = isNaN(saldoOrig) ? null : saldoOrig;
  state.settings.prazoOriginal = isNaN(prazoOrig) ? null : prazoOrig;
  saveState();
  render();
  modal.hidden = true;
});
